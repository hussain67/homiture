...
function MobileNavbar({ showMobileNav, setShowMobileNav }: { showMobileNav: boolean; setShowMobileNav: React.Dispatch<React.SetStateAction<boolean>> }) {
	return (
		<div className={`fixed w-[200px] bg-muted h-screen top-0  left-0 text-end ${showMobileNav ? "translate-x-0 opacity-95" : "-translate-x-full opacity-0"} transition-all duration-1000 `}>
			<button
